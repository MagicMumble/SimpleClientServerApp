...
func (client *Client) handleRequest(req *proto.Request) bool {
	...
	case "set new array":
		...
		if req.Data == nil {                
			...
			var mas string
			if err := json.Unmarshal(*req.Data, mas); err != nil {                     ////////!!!
				...
				if (mas == "") {
					...
					client.logger.Info("getting new array", "array", mas)
					client.arr = mas
				}
			}
		}
		if errorMsg == "" {
			...
			client.logger.Error("getting new array failed", "reason", errorMsg)
			client.respond("failed", errorMsg)
		}
	case "sort":
		if client.arr == "" {
			client.logger.Error("sorting of array failed", "reason", "array is absent")
			client.respond("failed", "array is absent")
		} else {
			client.respond("result", convert(client.arr))
		}
	default:
...
func normal(str string) bool {
	var mas []string 
	mas =strings.Split(str, " ")
	for i := 1; i<len(mas); i++ {
		log.Error("elem = ", mas[i])
		if _, err := strconv.Atoi(mas[i]); err != nil {
			return false
		}
	}
	return true
...
func convert(str string) string {
	var mas []string 
	mas =strings.Split(str, " ")
	array:=make([]int, 0, len(mas))
	var per int
	for i := 0; i<len(mas); i++ {
		per, _ = strconv.Atoi(mas[i])
		array = append(array, per) 
	}
	sort.Ints(array)
	var result string =""
	for i := 0; i<len(mas); i++ {
